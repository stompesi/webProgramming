...
// error_log( print_R($table_of_contents,TRUE) );
  class webeditorAdminController extends webeditor {
      ...
      foreach($output->data as $item) {
        ...
        if($item->type =="folder") {
          ...
          if(dirname($item->path) == $to_folder_path) {
            ...
            // $prev_parent_path = str_replace(' ', '\ ', $from_path);
    ...
    public function procWebeditorAdminSyncCodeBySourceCodeFolder() {
      $source_code_folder_path = sprintf('%ssource_code', $this->module_path);
      executeQuery('webeditor.deleteTableOfContents');
      $this->syncCodeBySourceCodeFolder("0", $source_code_folder_path);
    }

    // ------------------------

    function syncCodeBySourceCodeFolder($parent_table_of_content_srl, $source_code_folder_path) {
      $oWebEditorModel = getModel('webeditor');
      $oWebEditorAdminModel = getAdminModel('webeditor');
      $i = 0;
      foreach(glob($source_code_folder_path.'/*', GLOB_ONLYDIR) as $folder_path) {
        ...
        $oWebEditorAdminModel->insertTableOfContent($parent_table_of_content_srl, $i, $title, $folder_path, "folder");
        $output = $oWebEditorModel->getWebeditorTableOfContentByParentTableOfContentSrlAndLocation($parent_table_of_content_srl, $i);
        $this->syncCodeBySourceCodeFolder($output->data->table_of_content_srl, $folder_path);
        $i++;
      }
      foreach(glob($source_code_folder_path.'/*.{*}', GLOB_BRACE) as $file_path) {
        ...
        $oWebEditorAdminModel->insertTableOfContent($parent_table_of_content_srl, $i, $title, $file_path, "file");
